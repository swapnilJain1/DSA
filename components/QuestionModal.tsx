...
const QuestionModal: React.FC<ModalProps> = ({ 
  ...
  const [isEditingDesc, setIsEditingDesc] = useState(!initialQuestion.description); // Default to edit if empty
  ...
  useEffect(() => {
      if (isOpen) {
          ...
          // Set editing to true ONLY if it's a fresh/empty description on mount
          setIsEditingDesc(!initialQuestion.description); 
  ...
  const hasEdgeCases = data.description?.toLowerCase().includes('edge cases');
  ...
  const handleAutoDescription = async () => {
      ...
      try {
          ...
          setIsEditingDesc(false); // Show preview immediately
      } catch(e) { handleAIError(e); }
      setAiStatus(null);
  };
  ...
  const handleGenerateEdgeCases = async () => {
      ...
      try {
          ...
          // Switch to Preview Mode (Read Only) to show the rendered Markdown
          setIsEditingDesc(false);
          
          // Navigate/Scroll to the new section
          setTimeout(() => {
              if (descriptionWrapperRef.current) {
                  descriptionWrapperRef.current.scrollIntoView({ behavior: 'smooth', block: 'end' });
              }
          }, 150);
      } catch(e) { handleAIError(e); }
      setAiStatus(null);
  ...
  const HintsSection = () => (
      <div className="space-y-3 mt-6">
         <div className="flex justify-between items-center">
            ...
            {data.description && (!isMobile || !mobileReadOnly) && (
                <button 
         ...
         <div className="space-y-2">
            ...
            {data.hints.map((hint, i) => (
                <details key={i} className="group bg-yellow-500/10 border border-yellow-500/20 rounded-lg relative [&::-webkit-details-marker]:hidden">
                    ...
                    {(!isMobile || !mobileReadOnly) && <button onClick={() => handleRequestDeleteHint(i)} className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 text-slate-400 hover:text-red-500 transition-all z-10" title="Delete Hint"><Trash2 size={14} /></button>}
                </details>
            ))}
            {(!isMobile || !mobileReadOnly) && (isAddingHint ? (
            ...
            <div className="flex-1 overflow-y-auto p-6 custom-scrollbar space-y-6">
                ...
                <div className="space-y-4">
                    <div className="flex items-center gap-2">
                        ...
                        >
                            {aiStatus === 'Finding URL...' ? <Loader2 size={16} className="animate-spin" /> : <Search size={14}/>}
                        </button>
                    </div>
                        ...
                        />
                         {!data.title && (
                            <button 
                                ...
                                className="p-2 bg-indigo-500/10 text-indigo-400 hover:bg-indigo-500/20 rounded-lg transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                            >
                                {aiStatus === 'Fetching Title...' ? <Loader2 size={16} className="animate-spin" /> : <Wand2 size={14}/>}
                            </button>
                         )}
                         {data.url && (
                ...
                <div className="space-y-2">
                    <div className="flex justify-between items-center">
                        ...
                        <div className="flex gap-2">
                            {!hasEdgeCases && (
                                ...
                                >
                                    {aiStatus === 'Finding Edge Cases...' ? <Loader2 size={16} className="animate-spin" /> : <ShieldAlert size={10} />} Edge Cases
                                </button>
                            )}
                            <button 
                            ...
                            >
                                {aiStatus === 'Expanding...' ? <Loader2 size={16} className="animate-spin" /> : <Expand size={10} />} Expand
                            </button>
                            {!data.description && (
                                <button 
                                ...
                                >
                                    {aiStatus === 'Generating...' ? <Loader2 size={16} className="animate-spin" /> : <Wand2 size={10} />} Auto-Fill
                                </button>
                            )}
                        </div>
                    </div>
                    <div 
                        ...
                        className={`min-h-[200px] p-4 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 transition-colors cursor-text ${isEditingDesc ? 'ring-1 ring-indigo-500/50' : ''}`}
                    >
                         {isEditingDesc ? (
                              <textarea 
                                ref={descriptionRef}
        ...
        <div className="flex flex-col h-full bg-black/20" style={{ width: `${100 - leftPanelWidth}%` }}>
             ...
             <div className="h-14 border-b border-white/10 flex items-center justify-between px-4 bg-white/5 shrink-0">
                 ...
                 <div className="flex items-center gap-3">
                    ...
                    >
                        {aiStatus === 'Analyzing...' ? <Loader2 size={14} className="animate-spin" /> : <BrainCircuit size={14}/>} Coach
                    </button>
                    <button 
                           ...
                           <div className="flex gap-2">
                               {testCases.length === 0 && (
                                   <button 
                                    ...
                                    >
                                       {aiStatus === 'Generating Cases...' ? <Loader2 size={10} className="animate-spin"/> : <BrainCircuit size={12}/>} AI Cases
                                   </button>
                               )}
